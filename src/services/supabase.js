...
export const createOrder = async (orderData) => {
  try {
    ...
    if (orderData.items[0]?.id) {
      productsQuery = productsQuery.in('id', orderData.items.map(item => item.id));
    } else {
      productsQuery = productsQuery.in('name', orderData.items.map(item => item.name));
    }
    ...
    for (const item of updatedItems) {
      ...
      console.log(`Updating stock for ${item.name} from ${product.stock_quantity} to ${newStock}`);
